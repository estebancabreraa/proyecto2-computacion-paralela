...
// printf("ID: %d value: %ld ", id, block_size*(sub_range_iterations-1)+((sub_range_iterations-1)*(N-1)*block_size + block_size * (id+1)));
// if((block_size*(sub_range_iterations-1)+((sub_range_iterations-1)*(N-1)*block_size) + block_size * (id+1)) > upper){
//   sub_range_iterations = sub_range_iterations-1;
// }
// printf("Cada uno recorre: %ld y este %d tiene %ld iteraciones\n", range_per_node, id, sub_range_iterations);
// MPI_Finalize();
// return 0;
